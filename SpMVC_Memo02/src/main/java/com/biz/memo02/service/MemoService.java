...
public class MemoService {
	 ...
	 * 
	 * 1. MemoService 클래스의 어떤 method를 호출해서 사용할 곳에서 먼저 method 사용(호출) 코드를 작성 2. 당연히 문법
	 * 오류가 날 것이므로 3. 이클립스의 도움을 받아서 4. create method를 실행한다. 5. 이클립스는 MemoService 클래스에
	 * 자동으로 매개변수와 리턴값까지 설정하여 method를 작성 해 준다.
	 */
	...
	public int writeDB(MemoVO vo) {
		...
		int ret = 0;
		if (id > 0) {
			ret = memoMapper.update(vo);
		} else {
			ret = memoMapper.insert(vo);
		}
